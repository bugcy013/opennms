...
public class VEProviderGraphContainer implements GraphContainer, VertexListener, EdgeListener, ServiceListener {
    ...
    @Override
    public void vaadinUiInitialized(VaadinApplicationContext applicationContext) {
        m_sessionId = applicationContext.getSessionId();
        try {
            m_bundleContext.removeServiceListener(this);
            m_bundleContext.addServiceListener(this, String.format("(&(objectClass=%s)(sessionId=%s))", Criteria.class.getName(), m_sessionId));
